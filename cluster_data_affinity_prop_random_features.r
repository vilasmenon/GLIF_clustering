...
allclustgap=list()
for (nameval in rev(c("features_rand_g1","features_rand_g2","features_rand_g3","features_rand_g4"))) {
  set.seed(0)
  if (nameval=="features_rand_g1") {keepcols=sample(c(1,2,3,4,5,8,11,12,13,14),5)}
  if (nameval=="features_rand_g2") {keepcols=sample(c(1,2,3,4,5,8,11,12,13,14),7)}
  if (nameval=="features_rand_g3") {keepcols=sample(c(1,2,3,4,5,8,11,12,13,14),7)}
  if (nameval=="features_rand_g4") {keepcols=sample(c(1,2,3,4,5,8,11,12,13,14),9)}
  ...
  save(allclustgap,file="allclustgap_randomfeatures.rda")
}
load("allclustgap_randomfeatures.rda")
...
for (nameval in rev(c("features_rand_g1","features_rand_g2","features_rand_g3","features_rand_g4"))) {
  ...
  startmat=featfulldat_all
  newstart=scale(startmat[,apply(startmat,2,var)>0])
  numclust=which.max(allclustgap[[nameval]][[1]][3:20,3])+2 
  print(c(nameval,numclust))
  if (nameval=="features_rand_g1") {numclust=10}
  if (nameval=="features_rand_g2") {numclust=12}
  if (nameval=="features_rand_g3") {numclust=12}
  if (nameval=="features_rand_g4") {numclust=11}
