...
#for (nameval in rev(c("featuresnospike"))) {
  ...
  save(allclustgap,file="allclustgap.rda")
}
load("allclustgap.rda")

for (nameval in rev(c("featuresnospike","features","glif4","glif3","glif2","glif1"))) {
  if (nameval=="lif") {keepcols=c(1,3,4,5)}
  if (nameval=="glif1") {keepcols=c(1,3,4,5,8)}
  if (nameval=="glif2") {keepcols=c(1,3,4,5,8,9,10)}
  if (nameval=="glif3") {keepcols=c(2,3,4,5,6,7,8)}
